...
function createQueries( { db} ) {
    function byEmail(email) {
        return db.client('user_credentials')
        .where({email}).limit(1).then(camelCaseKeys).then(rows => rows[0])
    }
...
function createHandlers ({ actions }) {
    ...
    function handleRegisterUser(req, res, next){
        ...
        return actions.registerUser(req.context.traceId, attributes)
        .then(() => res.redirect(301, 'register/registration-complete'))
        .catch(ValidationError, err =>
            res.
              status(400)
              .render('register-users/templates/register',
              {userId: attributes.id, errors: err.errors})).catch(next)
    }
